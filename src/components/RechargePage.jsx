...
const RechargePage = () => {
  ...
  const handleConfirm = () => {
    if (!customAmount || !selectedChannel) {
      showAlert("Please select a valid amount and channel.", "warning");
      return;
    }
    ...
    navigate("/new-recharge", {
      state: { amount, channel, userId }
    });
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Main Content */}
        <Box sx={{ padding: 2 }}>
          <Typography fontWeight={600} fontSize="15px">Recharge amount</Typography>
          <TextField
            inputRef={inputRef}
            fullWidth
          ...
          <Box sx={{ backgroundColor: "#ffffff", borderRadius: 3, p: 2, mt: 3 }}>
            <Typography fontWeight={600} fontSize="15px" sx={{ mb: 1 }}>Recharge Channel</Typography>
              ...
              {paymentChannels.map((channel) => (
                <ListItem
                  ...
                  sx={{
                    ...
                    transition: "all 0.3s ease",
                    "&:hover": { backgroundColor: "#a5f2e7" },
                  }}
                >
                  <ListItemText
                    primary={<Typography fontWeight={700} fontSize="15px">{channel.name}</Typography>}
                    secondary={<Typography variant="body2" sx={{ color: "#666" }}>Amount Range: ₹{channel.minAmount} - ₹{channel.maxAmount}</Typography>}
                  />
          ...
          <Box sx={{ color: "#666", fontSize: "14px" }}>
            {[
              ...
              <Box key={index} sx={{ display: "flex", alignItems: "flex-start", mt: index === 0 ? 0 : 1 }}>
                ...
                <Typography variant="body2" sx={{ ml: 1 }}>{rule}</Typography>
              </Box>
            ))}
          </Box>
        </Box>
