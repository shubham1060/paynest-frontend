...
import { loginAdmin } from "../api/adminApi"; // Assuming you have a separate API for admin login
...
const AdminLoginForm = () => {
  ...
  const handleLogin = useCallback(async () => {
    ...
    try {
      const data = await loginAdmin(phone, password); // Login via admin API
      ...
      setTimeout(() => {
        navigate("/admin/dashboard"); // Redirect to the admin dashboard or wherever
      }, 2000);
    } catch (error) {
      if (error?.status === 401) {
          ...
          >
            <CardContent>
              <Box display="flex" justifyContent="center" mb={1}>
                <Avatar
                  src="/PN_logo.png" // New logo for admin login
                  alt="Company Logo"
